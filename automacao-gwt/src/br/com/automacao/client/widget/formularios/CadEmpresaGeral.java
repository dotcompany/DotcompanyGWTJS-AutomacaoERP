...
//	private DotTextField<String> 		tfIbgeUF = new DotTextField<String>();
	...
	private void buildFieldSetGeral() {
		...
		dfCadastro.buildInput(fsetGeral, "Data Cadastro", 106, 13, "110px", "22px", 2);
		dfCadastro.focus();
		cbTipoEmpresa.buildInput(fsetGeral, "Tipo da Empresa", 222, 14, "173px", "22px", EmpresaType.class, 3);
		cbConheceu.buildInput(fsetGeral, "Como nos Conheceu", 1, 58, "216px", "22px", ChegouType.class, 4);
		cbRamo.buildInput(fsetGeral, "Ramo de Atividade da empresa", 222, 58, "173px", "22p", RamoType.class, 5);
		cbEmitirNF.buildInput(fsetGeral, "Usar\u00E1 o sistema p/ Emitir NFe", 401, 58, "173px", "22px", EmitirNfType.class, 6);
	...
	private void buildFieldSetInformacoes() {
		...
		cbPessoa.buildInput(formInfoGeral, "Pessoa F.\\J.", 6, 13, "92px", "22px", PessoaType.class, 7);
		tfCpfCnpj.buildInput(formInfoGeral, "CPF\\CNPJ", 104, 13, "135px", "22px", 8);
		tfInscEstad.buildInput(formInfoGeral, "Inscri\u00E7\u00E3o Estadual", 245, 13, "99px", "22px", 9);
		tfInscMunic.buildInput(formInfoGeral, "Inscri\u00E7\u00E3o Municipal", 362, 13, "99px", "22px", 10);
		tfRazaoSocial.buildInput(formInfoGeral, "Nome\\Raz\u00E3o", 6, 57, "350px", "22px", 11);
		tfFantasia.buildInput(formInfoGeral, "Nome Fantasia", 362, 57, "333px", "22px", 12);
		tfNomeContato.buildInput(formInfoGeral, "Nome Contato", 6, 100, "350px", "22px", 13);
		...
		cbCodRegimeTrib.buildInput(formInfoComp, "C\u00F3digo de Regime Tribut\u00E1rio - CRT", 6, 13, "285px", "22px", ReceitaType.class, 26);
		cbTipoRegime.buildInput(formInfoComp, "Tipo de Regime", 297, 13, "188px", "22px", RegimeType.class, 27);
		tfPis.buildInput(formInfoComp, "Aliq. PIS", 491, 13, "82px", "22px", 28);
		tfConfins.buildInput(formInfoComp, "Aliq. CONFINS", 579, 13, "82px", "22px", 29);
		tfSuframa.buildInput(formInfoComp, "SUFRAMA", 6, 57, "82px", "22px", 30);
		tfInscEstadST.buildInput(formInfoComp, "Inscri\u00E7\u00E3o Estadual ST", 94, 57, "105px", "22px", 31);
	...
	private void buildFieldSetEndereco() {
		...
		tfEndereco.buildInput(fsetEndereco, "Endere\u00E7o", 88, 12, "336px", "22px", 15);
		...
		tfComplemento.buildInput(fsetEndereco, "Complemento", 0, 56, "160px", "22px", 19);
		tfBairro.buildInput(fsetEndereco, "Bairro", 166, 56, "142px", "22px", 20);
		tfCidade.buildInput(fsetEndereco, "Cidade", 314, 56, "150px", "22px", 21);
		cbUF.buildInput(fsetEndereco, "UF", 470, 56, "46px", "22px", EstadosBrType.class, 22);
		tfPais.buildInput(fsetEndereco, "País", 522, 56, "68px", "22px", 23);
		tfReferencia.buildInput(fsetEndereco, "Ponto de Refer\u00EAncia", 0, 99, "308px", "22px", 24);
		cbContato.buildInput(fsetEndereco, "Contatos (Telefones, Email, HomePage, Etc...)", 314, 99, "279px", "22px", ContatoType.class, 25);
...
//		cbContato.clear();
	...
	public void fillDTO(EmpresaMirror mirrorTarget) {
		...
		mirrorTarget.setComoChegou((ChegouType) cbConheceu.getValue().getItem());
		mirrorTarget.setTipoRamo((RamoType) cbRamo.getValue().getItem());
		mirrorTarget.setTipo((EmpresaType) cbTipoEmpresa.getValue().getItem());
		mirrorTarget.setTipoEmitirNota(((EmitirNfType) cbEmitirNF.getValue().getItem()));
		...
		// Informa��es Importantes -> Informa��es Complementares NFe
		mirrorTarget.setTipoReceita((ReceitaType) cbCodRegimeTrib.getValue().getItem());
		mirrorTarget.setTipoRegime((RegimeType) cbTipoRegime.getValue().getItem());
...
//		DesktopApp.getServiceGenerics().copyFields(fields, dto, this, new AsyncCallback<Void>() {
//			@Override
//			public void onFailure(Throwable caught) {
//				
//			}
...
//		});
		...
		if(dto.getComoChegou() != null)
			cbConheceu.setValue(new Combo(dto.getComoChegou().getNome(), dto.getComoChegou()));
		else
			cbConheceu.clear();
		
		if(dto.getTipoPessoa() != null)
			cbPessoa.setValue(new Combo(dto.getTipoPessoa().getNome(), dto.getTipoPessoa()));
		else
			cbPessoa.clear();
		
		if(dto.getTipoRamo() != null)
			cbRamo.setValue(new Combo(dto.getTipoRamo().getNome(), dto.getTipoRamo()));
		else
			cbRamo.clear();
		
		if(dto.getTipo() != null)
			cbTipoEmpresa.setValue(new Combo(dto.getTipo().getNome(), dto.getTipo()));
		else
			cbTipoEmpresa.clear();
		
		if(dto.getTipoEmitirNota() != null)
			cbEmitirNF.setValue(new Combo(dto.getTipoEmitirNota().getNome(), dto.getTipoEmitirNota()));
		else
			cbEmitirNF.clear();
		...
		if(dto.getTipoReceita() != null)
			cbCodRegimeTrib.setValue(new Combo(dto.getTipoReceita().getNome(), dto.getTipoReceita()));
		else
			cbCodRegimeTrib.clear();
		
		if(dto.getTipoRegime() != null)
			cbTipoRegime.setValue(new Combo(dto.getTipoRegime().getNome(), dto.getTipoRegime()));
		else
			cbTipoRegime.clear();
		
		tfPis.setValue(dto.getAliquotaPis() == null ? "" : dto.getAliquotaPis().toString());
		tfConfins.setValue(dto.getAliquotaCofins() == null ? "" : dto.getAliquotaCofins().toString());
		
		/* Endere�o */
		if (dto.getListaEndereco() != null && !dto.getListaEndereco().isEmpty()) {
			EnderecoMirror end = (new ArrayList<EnderecoMirror>(dto.getListaEndereco())).get(0);
