...
import br.com.automacao.server.repository.GridEditavelManagerBO;
...
@Service("gridEditavelManager")
public class GridEditavelServiceImpl extends BaseService implements GridEditavelService {
	...
	public ListUtil busca(String nameClass, List<String> idColumns) {
		try {
			...
			return new ListUtil(crud(GridEditavel.class).busca(clazz, idColumns), crud(Generics.class).count(EmpresaTO.class));
	...
	public int countFilterAll(String nameClass, String value, String[] nomeColunas, String[] tipoColunas) {
		try {
			return crud(GridEditavel.class).countFilterAll(Class.forName(nameClass), value, nomeColunas, tipoColunas);
	...
	public int countFilterLike(String nameClass, String[] like) {
		try {
			return crud(GridEditavel.class).countFilterLike(Class.forName(nameClass), like);
		} catch (ClassNotFoundException e) { throw new NegocioException("Classe não foi encontrada: " + e.getMessage()); }
	}
	...
	public FileColumn loadState(FileColumn fc) {
		...
		FileColumn fCol = GridEditavelManagerBO.getInstance().loadStateGridEditavel(nameClass, key);
		if(fCol != null)
			return fCol;
	...
	public void saveState(FileColumn fc) {
		GridEditavelManagerBO.getInstance().saveStateGridEditavel(fc);
	}
	...
	public ListUtil buscarTodos(String clazzName, Integer start, Integer maxResults, String[] idColumns) {
		...
		List<?> listBd = crud(GridEditavel.class).buscarTodos(clazzTO, start, maxResults, idColumns);
	...
	public ListUtil buscarFilter(String clazzName, Integer start, Integer maxResults, String[] idColumns, String[] like) {
		Class<?> clazz = ReflectionUtil.getClass(clazzName);
		Class<?> clazzTO = ReflectionUtil.getClasseAnotacao(clazz);
		List<?> listBd = crud(GridEditavel.class).buscarFilter(clazzTO, start, maxResults, idColumns, like);
		List<Mirror> listMirror = new ArrayList<Mirror>();
		copyList(listBd, listMirror);
		int rows = crud(GridEditavel.class).countFilterLike(clazzTO, like);
	...
	public ListUtil buscarFilterAll(String clazzName, Integer start, Integer maxResults, String value, String[] nomeColunas, String[] tipoColunas) {
		Class<?> clazz = ReflectionUtil.getClass(clazzName);
		Class<?> clazzTO = ReflectionUtil.getClasseAnotacao(clazz);
		List<?> listBd = crud(GridEditavel.class).buscarFilterAll(clazzTO, start, maxResults, value, nomeColunas, tipoColunas);
		List<Mirror> listMirror = new ArrayList<Mirror>();
		copyList(listBd, listMirror);
		int rows = crud(GridEditavel.class).countFilterAll(clazzTO, value, nomeColunas, tipoColunas);
		return new ListUtil(listMirror, rows);
	}
}